...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(final Student[] studentArray) {
        ...
        List<Student> studentsList = Arrays.asList(studentArray);
        double ageAverage = studentsList.parallelStream()
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
                List<Student> studentsList = Arrays.asList(studentArray);
        String mostCommon = null;
        Optional<Map.Entry<String, Long>> namesCount = studentsList.parallelStream()
                .filter(t -> !t.checkIsCurrent())
                .collect(Collectors.groupingBy(Student::getFirstName, Collectors.counting()))
                .entrySet().stream().max(Comparator.comparing(Map.Entry::getValue));
               
        mostCommon = namesCount.get().getKey();
        return mostCommon;        
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
                ...
                List<Student> studentsList = Arrays.asList(studentArray);
                Integer count = (int) studentsList.parallelStream()
                                    .filter(s -> !s.checkIsCurrent() && s.getAge() > 20 
                                    && s.getGrade() < 65).count();
