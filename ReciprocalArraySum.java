...
public final class ReciprocalArraySum {
    ...
    protected static double parArraySum(final double[] input) {
        ...
        int chunkStartInclusiveL = getChunkStartInclusive(0, 2, input.length);
        int chunkStartInclusiveR = getChunkStartInclusive(1, 2, input.length);
        int chunkEndExclusiveL = getChunkEndExclusive(0, 2, input.length);  
        int chunkEndExclusiveR = getChunkEndExclusive(1, 2, input.length); 
        ReciprocalArraySumTask l = new ReciprocalArraySumTask(chunkStartInclusiveL, chunkEndExclusiveL, input);
        ReciprocalArraySumTask r = new ReciprocalArraySumTask(chunkStartInclusiveR, chunkEndExclusiveR, input);
        l.fork();
        r.compute();   
        l.join();
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        subtasks [numTasks-1] = new ReciprocalArraySumTask(getChunkStartInclusive(numTasks-1, numTasks, input.length), 
        getChunkEndExclusive(numTasks-1, numTasks, input.length), input);

        for (int i = 0; i < numTasks-1; i++) {
            subtasks [i] = new ReciprocalArraySumTask(getChunkStartInclusive(i, numTasks, input.length), 
            getChunkEndExclusive(i, numTasks, input.length), input);  
            pool.execute(subtasks [i]);
        }         
       
        subtasks [numTasks-1].compute();

        sum += subtasks[numTasks-1].getValue(); 

        for (int i = 0; i < numTasks-1; i++) {
            subtasks [i].join();   
            sum += subtasks[i].getValue();                     
        }      
